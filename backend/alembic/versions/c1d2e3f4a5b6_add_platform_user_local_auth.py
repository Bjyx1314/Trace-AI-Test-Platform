...
并把 external_task_user_id 改为可空（本地账号无 SSO id）。
...
def upgrade() -> None:
    ...
    op.add_column('platform_users', sa.Column('auth_source', sa.String(length=20), nullable=False, server_default='external_task'))
    op.create_index('ix_platform_users_username', 'platform_users', ['username'], unique=True)
    op.alter_column('platform_users', 'external_task_user_id', existing_type=sa.String(length=200), nullable=True)


def downgrade() -> None:
    op.alter_column('platform_users', 'external_task_user_id', existing_type=sa.String(length=200), nullable=False)
